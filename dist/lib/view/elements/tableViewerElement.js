...
export class TableViewerElement extends BaseElement {
    ...
    constructor(pc, fc, table, maxWidth) {
        ...
        if (table.columns) {
            ...
            logger.info(`Table '${this.name}' has no design-time columns. It will be rendered based on the first row of the data received at run time`);
        }
    }
    ...
    getDynamicHeader(data, names) {
        ...
        for (const name of names) {
            ...
            if (d) {
                ...
                logger.error(`Column '${name}' is requested at run time, but that is not a valid field/child/column as per design time parameters.
    ...
    initConfig() {
        if (!this.configEle) {
            if (this.configurable) {
                logger.error('HTML template for table-view does not have a container with data-id="list-config" but the table is marked for dynamic configuration');
        ...
        const configEle = elementFactory.newElement(this.pc, fc, panel, this.maxWidth);
    ...
    initSearch() {
        if (!this.searchEle) {
            if (this.searchable) {
                logger.error(`Table Viewer Element has no element with data-search, implying that it is not designed for quick search feature. 
        ...
        if (tagName === 'INPUT') {
            ...
            if (!input) {
                logger.error(`html template for table-viewer has no input element in its search element`);
                this.searchable = false;
                return;
            }
        }
    ...
    sort(column) {
        ...
        if (!th) {
            logger.error(`This table has no column named ${column}. sort command ignored`);
            return;
        }
