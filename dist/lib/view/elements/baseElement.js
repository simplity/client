...
export class BaseElement {
    ...
    maxWidth) {
        ...
        if (comp.label) {
            ...
            if (this.labelEle) {
                ...
                logger.info(`node ${comp.name} has a label value of "${comp.label}" but the html has no element with data-id="label"`);
            }
        }
        /**
        ...
        if (maxWidth !== 0) {
            ...
            if (width > maxWidth) {
                logger.error(`Page element '${this.name}' specifies a width of ${width} but the max possible width is only ${maxWidth};
     ...
     */
    setError(msg) {
        logger.warn(`component type ${this.comp.compType} has not implemented setError(), but a request is received with value="${msg}"`);
