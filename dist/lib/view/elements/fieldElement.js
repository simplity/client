...
export class FieldElement extends BaseElement {
    ...
    constructor(pc, fc, field, maxWidth, initialValue) {
        ...
        if (val !== undefined) {
            console.info(`Setting default initial value of ${val} to field ${this.name}`);
            this.setValue(val);
            if (this.fc) {
    ...
    resetValue() {
        let value = this.field.initialValue;
        if (value === undefined) {
            value = '';
        }
        this.setValue(value);
    ...
    setValue(newValue) {
        ...
        switch (this.fieldRendering) {
            ...
            case 'hidden':
                return;
            default:
                logger.error(`field rendering type ${this.fieldRendering} not implemented. Value not set to field ${this.name}`);
        }
    }
    ...
    validate() {
        ...
        if (!this.textValue) {
            ...
            if (this.field.isRequired) {
                msgs = [this.createMessage(conventions.messageIds.valueIsRequired)];
            }
        }
        else {
    ...
    setList(list) {
        ...
        if (this.textValue) {
            ...
            logger.error(`Field ${this.name}: Current value is '${this.textValue}'. However, this is not a valid value in the new list being set.`);
            this.value = '';
            this.textValue = '';
            this.valueHasChanged('');
            return;
    ...
    setError(text) {
        if (!text) {
            ...
            this.errorMessage = '';
            if (this.descEle) {
                this.descEle.innerText = this.field.helpText || '';
            }
            htmlUtil.setViewState(this.root, 'invalid', false);
            return;
        }
        if (this.errorMessage) {
            this.errorMessage += ';\n' + text;
        ...
        if (this.descEle) {
            ...
            logger.info(`field ${this.name} is invalid with an error message="${this.errorMessage}". The field rendering has no provision to show error message`);
        }
        htmlUtil.setViewState(this.root, 'invalid', true);
    }
    /**
     * overriding to apply disabled state to the input element
     * @param stateName
     * @param stateValue
     */
