...
export class PanelElement extends BaseElement {
    ...
    constructor(pc, fc, panel, maxWidth) {
         ...
         */
        if (panel.formName) {
            const form = this.ac.getForm(panel.formName);
            if (fc) {
                fcForChildren = fc.newFormController(this.name, form);
            }
            else {
                logger.warn(`panel "${this.name}" has a formName, but this panel itself is not controlled by a form. The child form-controller will be made a child of the root-form-controller fo this page`);
        ...
        for (const child of panel.children) {
            const ele = elementFactory.newElement(this.pc, fcForChildren, child, maxWidth);
