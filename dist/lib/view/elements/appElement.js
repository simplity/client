...
export class AppElement {
    ...
    navigate(options) {
        ...
        if (!menu.pageName) {
            logger.error(`Menu item ${options.menuItem} has no pageName. Can not navigate`);
            return;
        }
    ...
    renderAsPopup(panel) {
        ...
        logger.warn(`Panel ${panel.name} to be rendered as popup. This functionality is not yet developed`);
    }
    closePopup() {
        if (this.currentPopup) {
