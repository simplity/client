import { FilterCondition, SortBy, StringMap } from '../common';
...
export type ViewPage = PageTemplate & {
   ...
   */
  tabs?: { name: string; label: string; icon?: string; fields: string[] }[];
};
...
export type SavePage = PageTemplate & {
   ...
   * in case fields have to be grouped into tabs
   */
  tabs?: { name: string; icon?: string; label: string; fields: string[] }[];
};
